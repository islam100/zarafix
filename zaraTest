...
/* Logo */
.logo {
  font-size:3rem; font-weight:900; color:#ff4081;
  text-shadow: 3px 3px 5px #fff, 6px 6px 8px rgba(0,0,0,0.3);
  user-select:none;
}
...
button.btn-login { background: linear-gradient(45deg,#00bcd4,#009688); color:#fff; }
button.btn-login:hover { background: linear-gradient(45deg,#009688,#00796b); }
button.btn-logout { background: linear-gradient(45deg,#ff4081,#e91e63); color:#fff; }
button.btn-logout:hover { background: linear-gradient(45deg,#e91e63,#c2185b); }
...
.property-card:hover { box-shadow:0 0 14px rgba(0,0,0,0.25); }
.property-card img { width:100%; height:130px; object-fit:contain; border-radius:6px; }
.property-card .info { display:flex; flex-direction:column; gap:2px; font-size:0.85rem; line-height:1.2em; }

/* WhatsApp button */
.btn-whatsapp { background:#25d366; color:white; padding:4px 8px; border-radius:6px; text-decoration:none; display:inline-block; text-align:center; font-weight:600; font-size:0.8rem; }
...
#modalContent { background:white; width:95vw; max-width:600px; max-height:95vh; overflow-y:auto; border-radius:10px; padding:20px; position:relative; box-sizing:border-box; }
...
function renderProperties(posts){
  ...
  posts.forEach(({key,post})=>{
    ...
    const title=document.createElement('p'); title.textContent=post.title; infoDiv.appendChild(title);
    const city=document.createElement('p'); city.textContent="City: "+post.city; infoDiv.appendChild(city);
    const price=document.createElement('p'); price.textContent="₹"+formatPrice(post.price); infoDiv.appendChild(price);
    card.appendChild(infoDiv);

    const btnGroup=document.createElement('div'); btnGroup.className="btn-group";
    const waBtn=document.createElement('a'); waBtn.href=getWhatsAppLink(post); waBtn.target="_blank"; waBtn.className="btn-whatsapp"; waBtn.textContent="WhatsApp"; btnGroup.appendChild(waBtn);
...
auth.onAuthStateChanged(user=>{
  currentUser=user;
  if(user){ loginBtn.style.display="none"; logoutBtn.style.display="inline-block"; isAdmin=(user.email===adminEmail); fetchProperties(); }
  else{ loginBtn.style.display="inline-block"; logoutBtn.style.display="none"; isAdmin=false; propertyList.innerHTML="<p style='text-align:center; color:#666;'>Login करें ताकि properties देख सकें।</p>"; }
});
